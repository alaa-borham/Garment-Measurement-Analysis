...
import { Loader2, GitCompare, X, ExternalLink, Maximize2, Minimize2 } from "lucide-react";
...
function DatasetPanel({
  ...
  return (
    <Card className="flex flex-col overflow-hidden">
      ...
      {/* Body — iframe to dataset page in embed mode */}
      <div className={`relative bg-background ${isFullscreen ? "h-[calc(100vh-200px)]" : "h-[700px]"}`}>
        {!dsId && (
          ...
          <>
            {iframeLoading && (
              <div className="absolute inset-0 flex flex-col items-center justify-center gap-2 bg-background z-10">
                <Loader2 className="w-8 h-8 animate-spin text-primary" />
                <span className="text-xs text-muted-foreground">
                  {isAr ? "جارٍ تحميل لوحة التحليل..." : "Loading workspace..."}
                </span>
              </div>
            )}
            <iframe
...
export default function CompareDatasetsPage() {
  ...
  // إذا كان هناك لوحة موسعة، اعرضها فقط
  const visiblePanels = fullscreenIdx !== null
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-1">
            {isAr
              ? "كل لوحة تعرض الواجهة الكاملة (استعراض، تحليل، Pivot، مخطط، تحليل متقدم) لملف مختار"
              : "Each panel shows the full workspace (Explore, Analyze, Pivot, Chart, Advanced) for a selected dataset"}
          </p>
        </div>
        <Button
          onClick={addPanel}
